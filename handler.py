...
def save_input_image(image_input):
    ...
    if isinstance(image_input, str) and (image_input.startswith('http://') or image_input.startswith('https://')):
        # Download from URL
        logger.info(f"Downloading image from URL: {image_input[:80]}...")
        req = urllib.request.Request(image_input, headers={'User-Agent': 'Mozilla/5.0'})
        with urllib.request.urlopen(req, timeout=60) as response:
            image_data = response.read()
        with open(dest_path, 'wb') as f:
            f.write(image_data)
        logger.info(f"Image downloaded and saved: {filename} ({len(image_data)} bytes)")
    elif isinstance(image_input, str) and len(image_input) > 200:
        # Treat as base64
        ...
        with open(dest_path, 'wb') as f:
            f.write(image_data)
        logger.info(f"Base64 image decoded and saved: {filename} ({len(image_data)} bytes)")
    else:
        # Treat as file path
        src_path = image_input
        logger.info(f"Using image from path: {src_path}")
        ...
        with open(dest_path, 'wb') as f:
            f.write(image_data)
        logger.info(f"Image copied to input dir: {filename} ({len(image_data)} bytes)")
...
def handler(job):
    ...
    try:
        ...
        logger.info(f"Task {task_id}: Processing image-to-video with prompt: '{prompt[:80]}...'")
        logger.info(f"Parameters - frame_count: {frame_count}, seed: {seed}")
        ...
        # Image input (LoadImage node 98) - just the filename, not full path
        workflow["98"]["inputs"]["image"] = image_filename

        # Positive prompt
        workflow["92:3"]["inputs"]["text"] = prompt

        # Negative prompt (optional - use default if not provided)
        if negative_prompt is not None:
            workflow["92:4"]["inputs"]["text"] = negative_prompt

        # Frame count (PrimitiveInt node 92:62)
        workflow["92:62"]["inputs"]["value"] = frame_count

        # Seeds for both stages
